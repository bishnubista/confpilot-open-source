...
function OrganizerReviewWorkspace({ eventSlug, preferredProposalId, onUnauthorized }: { eventSlug: string; preferredProposalId?: string; onUnauthorized: () => void }) {
  ...
  return <main className="page review-admin-page"><PageHeader eyebrow="Program · Abstracts" title="Review operations" description="Assign reviewers, monitor scorecards, record one final decision, then save its notification snapshot to the outbox separately." /><EvaluationPlanManager eventSlug={eventSlug} onUnauthorized={onUnauthorized} /><div className="review-live-warning" role="note"><strong>Assign after the CFP closes.</strong><span>Reviewers see the current submitted dossier; rolling-review snapshots are not enabled yet.</span></div>{proposals.length === 0 ? <section className="section-card supporting-state"><div><h2>No submitted proposals are ready for review.</h2><p>Submitted proposals appear here without exposing drafts.</p></div></section> : <div className="review-admin-layout"><section className="section-card review-progress-list" aria-label="Proposal review progress"><div className="card-heading"><div><p className="overline">Review queue</p><h2>{proposals.length} proposals</h2></div><div className="review-results-actions"><label>Sort by aggregate score<select value={scoreSort} onChange={(event) => setScoreSort(event.target.value as typeof scoreSort)}><option value="submission">Submission order</option><option value="descending">Highest score first</option><option value="ascending">Lowest score first</option></select></label><button type="button" className="button button-outline" disabled={exportState === 'downloading'} onClick={() => void exportReviews()}>{exportState === 'downloading' ? 'Preparing CSV…' : 'Export review results (CSV)'}</button></div></div><p className="builtin-note">Aggregates normalize submitted scorecards to a comparable 1–5 scale. Select a proposal to see whether each submitted score came from a named plan version or the legacy originality and relevance scorecard.</p>{exportState === 'success' && <p className="download-success" role="status">Downloaded {eventSlug}-review-results.csv.</p>}{exportState === 'error' && <p className="error-text" role="alert">Could not download review results. Refresh and try again.</p>}{sortedProposals.map((proposal) => <ReviewProgressButton eventSlug={eventSlug} key={proposal.proposalId} proposal={proposal} selected={proposal.proposalId === selectedId} onSelect={() => setSelectedId(proposal.proposalId)} />)}</section>{selected && <OrganizerReviewDetail eventSlug={eventSlug} key={selected.proposalId} proposal={selected} reviewers={reviewers.data.reviewers} initialDecision={selectedDecision} onChanged={progress.reload} onDecisionChanged={decisions.reload} onUnauthorized={onUnauthorized} />}</div>}<DecisionLedger eventSlug={eventSlug} decisions={decisions.data.decisions} activeProposalId={selected?.proposalId} onChanged={decisions.reload} onUnauthorized={onUnauthorized} /></main>
}
