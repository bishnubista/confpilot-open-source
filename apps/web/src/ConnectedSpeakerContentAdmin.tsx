...
import { asApiError, isAccessError } from './session'
...
function SpeakerCommunicationPanel({
  ...
  const queue = async (event: SubmitEvent) => {
    ...
    try {
      ...
      setMessage(`${result.queuedCount} immutable message${result.queuedCount === 1 ? '' : 's'} queued${skippedDetails.length ? ` · Skipped ${skippedDetails.join('; ')}` : ''}. Queueing is not proof of sending or delivery.`)
  ...
  return <section className="section-card communication-control" aria-labelledby="speaker-communication-heading"><div className="card-heading"><div><p className="overline">Speaker communications</p><h2 id="speaker-communication-heading">Exact-audience message queue</h2></div><span className="count-pill">{selected.length} selected</span></div><p className="resource-note">Select individual speakers or the current filtered result. ConfPilot creates one immutable queue snapshot per selected recipient and never expands the audience afterward. Provider delivery is at-least-once, so a rare duplicate remains possible after an ambiguous provider result. Use this for conference operations, not marketing newsletters.</p><div className="communication-selection-actions"><button type="button" className="button button-outline" disabled={visibleSpeakerIds.length === 0} onClick={toggleVisibleSelection}>{visibleSelected ? 'Clear visible selection' : `Select filtered (${visibleSpeakerIds.length})`}</button><button type="button" className="button button-outline" disabled={selectedSpeakerIds.size === 0} onClick={() => onSelection(new Set())}>Clear all</button><span>{selected.map(speakerIdentityLabel).join(', ') || 'No recipients selected'}</span></div><form className="communication-compose" onSubmit={(event) => void queue(event)}><label>Subject<input required maxLength={998} value={subject} onChange={(event) => setSubject(event.target.value)} /></label><label>Message<textarea required maxLength={20_000} value={body} onChange={(event) => setBody(event.target.value)} /></label><p className="resource-note">Merge fields: <code>{'{first_name}'}</code> <code>{'{session_title}'}</code> <code>{'{portal_link}'}</code>. ConfPilot resolves them from each selected speaker’s canonical record.</p>{previews.length > 0 && (subject || body) && <section className="communication-preview" aria-label="Per-recipient message preview"><p className="overline">Per-recipient preview</p>{previews.map((preview) => <article key={preview.speaker.profile.id}><strong>{preview.speaker.profile.name} · {preview.subject || 'No subject yet'}</strong><pre>{preview.body}</pre></article>)}{selected.length > previews.length && <small>Previewing 3 of {selected.length} recipients.</small>}</section>}<button className="button button-primary" disabled={pending || selected.length === 0}>{pending ? 'Queueing snapshots…' : `Queue ${selected.length || ''} message${selected.length === 1 ? '' : 's'}`.replace('  ', ' ')}</button></form>{error && <ErrorNotice error={error} />}{message && <p className="save-state" role="status">{message}</p>}<div className={`communication-capability ${capability?.enabled ? 'enabled' : 'disabled'}`}><strong>{capability?.enabled ? `Scheduled provider dispatch enabled · ${capability.provider}` : 'Provider dispatch disabled'}</strong><span>{capability?.enabled ? `Only snapshots created at or after ${dateTime(capability.sendAfter)} are eligible. Provider acceptance is recorded; inbox delivery remains unverified.` : 'Queued records remain untouched until an operator explicitly enables a valid binding, sender, and activation cutoff.'}</span></div><div className="communication-history"><div className="card-heading"><div><p className="overline">Immutable history</p><h3>Recent messages</h3></div>{history.status === 'success' && <span className="count-pill">{history.data.messages.length}</span>}</div>{history.status === 'loading' && <p role="status">Loading communication history…</p>}{history.status === 'error' && !isAccessError(history.error) && <ErrorNotice error={asApiError(history.error)} reload={history.reload} />}{history.status === 'success' && history.data.messages.length === 0 && <p>No communication snapshots yet.</p>}{history.status === 'success' && history.data.messages.map((item) => <article key={item.id}><div><strong>{item.subject}</strong><span>{item.recipient.name} · {item.recipient.email}</span><small>{dateTime(item.createdAt)} · {item.intent.replaceAll('_', ' ')}</small></div><div><span className={`status-badge communication-${item.transportStatus}`}>{communicationStatusLabel(item.transportStatus)}</span><small>{item.attemptCount} provider attempt{item.attemptCount === 1 ? '' : 's'}{item.lastErrorCode ? ` · ${item.lastErrorCode}` : ''}</small></div></article>)}</div></section>
}
...
function SpeakerRoster({ onUnauthorized, preferredSpeakerId }: { onUnauthorized: (error: ApiError) => void; preferredSpeakerId?: string }) {
  ...
  useEffect(() => {
    if (preferredSpeakerId) setSelectedId(preferredSpeakerId)
  }, [preferredSpeakerId])
  ...
  const selected = rows.find((item) => item.profile.id === selectedId) ?? rows[0]
  ...
  return <main className="page"><PageHeader eyebrow="Program · Speakers" title="Speaker readiness" description="Live profile, task, deliverable, approval, visibility, and communication state." />{rosterError && <ErrorNotice error={rosterError} reload={resource.reload} />}<SpeakerRosterIngest reload={resource.reload} onUnauthorized={onUnauthorized} /><div className="speaker-roster-filters"><label className="search"><span aria-hidden="true">⌕</span><input aria-label="Search speakers" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search speakers" /></label><label>Speaker readiness<select value={filter} onChange={(event) => setFilter(event.target.value as typeof filter)}><option value="all">All speakers</option><option value="ready">Ready</option><option value="attention">Needs attention</option></select></label><label>Workflow status<select value={workflow} onChange={(event) => setWorkflow(event.target.value as typeof workflow)}><option value="all">All statuses</option><option value="invited">Awaiting confirmation</option><option value="confirmed">Confirmed</option><option value="declined">Declined</option></select></label><p className={`active-content-filter filter-${filter}`} role="status" aria-label="Active readiness filter">{filterLabel} · {rows.length} of {roster.speakers.length}</p></div><SpeakerCommunicationPanel speakers={roster.speakers} visibleSpeakerIds={rows.map((item) => item.profile.id)} selectedSpeakerIds={selectedSpeakerIds} onSelection={setSelectedSpeakerIds} onUnauthorized={onUnauthorized} /><section className="section-card connected-speaker-roster"><div className="table-wrap" role="region" aria-label="Speaker readiness roster" tabIndex={0}><table><thead><tr><th scope="col">Select</th>{['Speaker', 'Profile', 'Release task', 'Headshot', 'Bio', 'Task progress', 'Deliverables', 'Approval', 'Visibility', 'Next due'].map((label) => <th scope="col" key={label}>{label}</th>)}</tr></thead><tbody>{rows.map((item) => { const identity = speakerIdentityLabel(item); return <tr key={item.profile.id} data-speaker-id={item.profile.id} className={selected?.profile.id === item.profile.id ? 'selected-row' : ''}><td><input type="checkbox" aria-label={`Select ${identity} for communication`} checked={selectedSpeakerIds.has(item.profile.id)} onChange={() => toggleSpeaker(item.profile.id)} /></td><td><button type="button" className="person-cell" aria-label={`Open ${identity}`} onClick={() => setSelectedId(item.profile.id)}><span className="avatar avatar-soft">{item.profile.name.split(' ').map((part) => part[0]).join('').slice(0, 2)}</span><span><strong>{item.profile.name}</strong><small>{item.profile.contactEmail ?? `No contact email · ID ${item.profile.id}`}</small><small>{item.sessions.length} {item.sessions.length === 1 ? 'session' : 'sessions'}</small></span></button></td><td><ReadinessMark value={item.readiness.profileReady} /></td><td><ReadinessMark value={item.readiness.agreementReady} /></td><td><ReadinessMark value={item.readiness.headshotReady} /></td><td><ReadinessMark value={Boolean(item.profile.bio)} /></td><td><TaskProgress tasks={item.tasks} /></td><td><ReadinessMark value={item.readiness.deliverablesReady} /></td><td>{item.sessions.every((session) => session.approvalStatus === 'approved') ? 'Approved' : 'Pending'}</td><td>{item.profile.publicVisibility}</td><td>{dateTime(item.readiness.nextDueAt)}</td></tr> })}</tbody></table></div>{rows.length === 0 && <div className="empty-state"><h3>No speakers match</h3><p>Change the search, speaker-readiness, or workflow filter.</p></div>}</section>{selected && <SpeakerDetail key={selected.profile.id} item={selected} reload={resource.reload} onUnauthorized={onUnauthorized} />}</main>
}
...
function ContentDossier({ session, reload, onUnauthorized }: { session: OrganizerContentDossier; reload: () => void; onUnauthorized: (error: ApiError) => void }) {
  const eventSlug = useEventSlug()
  const [pending, setPending] = useState('')
  const [error, setError] = useState<ApiError | null>(null)
  const [message, setMessage] = useState('')
  ...
  return <div className="content-dossier-connected" aria-busy={Boolean(pending)}><header className="section-card content-readiness-summary"><div><p className="overline">Accepted session · {session.track}</p><h2>{session.title}</h2><p>{session.presenters.map((speaker) => speaker.name).join(', ')}</p></div><span className={`status-badge ${approved ? 'status-live' : 'status-draft'}`}>{session.approvalStatus.replace('_', ' ')}</span><div className="approval-gates" id={gatesId}><strong>{session.unmetApprovalGates.length === 0 ? 'All approval gates satisfied' : `${session.unmetApprovalGates.length} approval gates remain`}</strong>{session.unmetApprovalGates.length > 0 && <ul>{approvalGateLabels.map((gate) => <li key={gate}>{gate}</li>)}</ul>}</div><div className="content-approval-action"><button className={approved ? 'button button-outline' : 'button button-primary'} disabled={Boolean(pending) || approvalBlocked} aria-describedby={gatesId} title={approvalBlocked ? approvalGateTitle : undefined} onClick={() => void mutate('approval', () => organizerSpeakerContentApi.approval(eventSlug, session.id, { approvalStatus: approved ? 'pending' : 'approved', expectedRevision: session.revision }))}>{pending === 'approval' ? 'Saving approval…' : approved ? 'Move approval to pending' : 'Approve content'}</button>{approvalBlocked && <small>Resolve every listed gate before approval.</small>}</div></header><SessionEditor key={`${session.id}:${session.revision}`} session={session} mutate={mutate} /><section className="section-card content-files-panel"><div className="card-heading"><div><p className="overline">Durable requests</p><h2>Files and versions</h2></div><span className="count-pill">{session.versions.length}</span></div>{session.requests.map((request) => <article className="content-request-row" key={request.id}><div><strong>{request.label}</strong><small>{request.active ? 'Active' : 'Inactive'} · due {dateTime(request.dueAt)} · {request.required ? 'required' : 'optional'}</small></div><button className="button button-outline" onClick={() => void mutate(request.id, () => organizerSpeakerContentApi.updateRequest(eventSlug, session.id, request.id, { label: request.label, instructions: request.instructions, dueAt: request.dueAt, allowedContentTypes: request.allowedContentTypes, maxBytes: request.maxBytes, required: request.required, active: !request.active, revision: request.revision }))}>{request.active ? 'Deactivate' : 'Activate'}</button></article>)}<RequestCreator session={session} mutate={mutate} /><ReviewControls session={session} mutate={mutate} /></section><ContentCommentComposer session={session} mutate={mutate} /><section className="section-card content-history"><p className="overline">Immutable audit</p><h2>Session history</h2><p className="history-restore-help">Each entry is the complete snapshot saved immediately before the recorded change. Restoring replaces the current title, abstract, track, format, and duration with the values shown; the current version is preserved as a new audit entry.</p>{session.history.length === 0 && <p>No content edits yet.</p>}{session.history.slice().reverse().map((history) => <article key={history.id}><div><span className="history-snapshot-kind">{history.action === 'restored' ? 'Snapshot before restore' : 'Snapshot before edit'}</span><strong>{history.title}</strong><small>{history.actorName} · {dateTime(history.createdAt)} · Change recorded: {history.changeNote}</small><p className="history-snapshot-abstract"><b>Abstract snapshot</b><span>{history.abstract}</span></p><small>{history.track} · {history.format} · {history.durationMinutes} minutes</small></div><button className="button button-outline" onClick={() => void mutate(history.id, () => organizerSpeakerContentApi.restoreSession(eventSlug, session.id, history.id))}>Restore this snapshot</button></article>)}</section>{error && <ErrorNotice error={error} reload={error.status === 409 ? reload : undefined} />}<p className="save-state" aria-live="polite">{message}</p></div>
}
...
function ContentOperations({ onUnauthorized, preferredSessionId }: { onUnauthorized: (error: ApiError) => void; preferredSessionId?: string }) {
  ...
  useEffect(() => { if (resource.status === 'error' && isAccessError(resource.error)) onUnauthorized(asApiError(resource.error)) }, [onUnauthorized, resource.error, resource.status])
  if (resource.status === 'loading') return <main className="page" role="status"><PageHeader eyebrow="Program · Content" title="Deliverables and approvals" description="Loading accepted-session dossiers…" /></main>
  ...
  const selected = resource.data.sessions.find((session) => session.id === selectedId) ?? resource.data.sessions[0]
  return <main className="page"><PageHeader eyebrow="Program · Content" title="Deliverables and approvals" description="Review the live accepted dossier, immutable versions, and exact approval gates." action={<a className="button button-dark" href={resource.data.approvedDeliverablesArchivePath}>Download approved files (.zip)</a>} /><ContentLibrary sessions={resource.data.sessions} /><div className="content-operations-layout"><aside className="section-card content-session-list"><div className="card-heading"><div><p className="overline">Accepted sessions</p><h2>Dossiers</h2></div><span className="count-pill">{resource.data.sessions.length}</span></div>{resource.data.sessions.map((session) => <button type="button" className={selected?.id === session.id ? 'active' : ''} key={session.id} onClick={() => setSelectedId(session.id)}><strong>{session.title}</strong><small>{session.deliverablesStatus} · {session.approvalStatus.replace('_', ' ')}</small></button>)}</aside>{selected ? <ContentDossier session={selected} reload={resource.reload} onUnauthorized={onUnauthorized} /> : <section className="section-card empty-state"><h2>No accepted sessions</h2><p>Accepted sessions will appear after decisions are recorded.</p></section>}</div></main>
}
