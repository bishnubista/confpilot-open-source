...
function AgendaWorkspace({ eventSlug, onUnauthorized, preferredSessionId }: { eventSlug: string; onUnauthorized: (error: ApiError) => void; preferredSessionId?: string }) {
  ...
  const programIsCurrent = data.event.status === 'published' && placedNotPublicCount === 0
  const publishLabel = pending === 'publish'
    ? 'Publishing…'
      : data.event.status !== 'published'
      ? 'Publish program'
      : awaitingPublicationCount > 0
        ? `Publish ${awaitingPublicationCount} ${awaitingPublicationCount === 1 ? 'update' : 'updates'}`
        : placedNotPublicCount > 0
          ? 'Retry publication'
